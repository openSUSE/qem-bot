...
class Submission:
    ...
    def __init__(self, submission: dict[str, Any]) -> None:
        ...
        self.type: str = submission.get("type") or settings.default_submission_type
        ...
        for r in submission["channels"]:
            ...
            if "all" in settings.obs_products_set or product in settings.obs_products_set:
    ...
    def create(cls, data: dict) -> Submission | None:
        ...
        sub_id = f"{data.get('type') or settings.default_submission_type}:{data.get('number')}"
        try:
            return cls(data)
