...
def read_utf8(name: str) -> str:
    with pathlib.Path("responses/{}".format(name)).open(encoding="utf8") as utf8:
        return utf8.read()


def read_json(name: str) -> Any:
    with pathlib.Path("responses/{}.json".format(name)).open(encoding="utf8") as json_file:
        return json.loads(json_file.read())


def read_xml(name: str) -> ET.ElementTree:
    return ET.parse("responses/{}.xml".format(name))
...
def get_product_name_and_version_from_scmsync(scmsync_url: str) -> tuple[str, str]:
    m = re.search(r".*/products/(.*)#(.*)", scmsync_url)
...
def add_comments_and_referenced_build_results(
    ...
    for comment in reversed(comments):
        ...
        if user_name == "autogits_obs_staging_bot":
            add_build_results(incident, re.findall(r"https://[^ ]*", body), dry)
            break
