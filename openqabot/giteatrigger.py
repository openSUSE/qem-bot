...
ISO_REGEX = r"SLES-(?P<version>[\d\.]+)-Online-x86_64-Build(?P<build>[0-9\.]+)\.install.iso$"
...
class GiteaTrigger:
    ...
    def __init__(self, args: Namespace) -> None:
        """Iinitialize GiteaTrigger class.
        ...
        self.pr_label: str = args.pr_label
    ...
    def is_openqa_triggering_needed(self, version: str, build: str) -> bool:
    ...
    def check_pullrequest(self, pullrequest: PullRequest) -> None:
        ...
        rez = Crawler(verify=True).get_regex_match_from_url(repo_url, ISO_REGEX)

        if rez:
            version = rez[1]
            build = f"PR-{pullrequest.number}-{rez[2]}"
            if self.is_openqa_triggering_needed(version, build):
                openqa_settings = {
                    "ISO_URL": f"{repo_url}{rez[0]}",
            ...
            log.warning("No ISO found for %s in %s", self.arch, repo_url)
    ...
    def get_prs_by_label(self) -> None:
        ...
        for pr in open_prs:
            ...
            try:
                ...
                if pullrequest.has_label(self.pr_label):
