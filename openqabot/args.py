...
DEFAULT_GITEA_PROJECT = str(_default("gitea_project"))
...
def _require_single_project(projects: list[str], pr_number: int | None) -> None:
    ...
    if pr_number and len(projects) > 1:
        typer.echo(
...
class GiteaSyncOptions:
    ...
    amqp: bool = False
    amqp_url: str = str(_default("amqp_url"))
...
def _run_gitea_sync(args: SimpleNamespace, options: GiteaSyncOptions) -> int:
    ...
    projects = _split_projects(options.gitea_project)
    _require_single_project(projects, options.pr_number)
    ...
    if (
        ret := _run_gitea_sync(
            ...
            GiteaSyncOptions(
                ...
                pr_number=pr_number,
                amqp=amqp,
                amqp_url=amqp_url or config_module.settings.amqp_url,
...
def increment_approve(  # ruff: ignore[too-many-arguments]
    ...
    *,
    ...
    consider_standalone_jobs: Annotated[
        bool,
        typer.Option(
            "--consider-standalone-jobs/--no-consider-standalone-jobs",
