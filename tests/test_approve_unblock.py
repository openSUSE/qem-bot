    ...
    responses.add(
        ...
        json={
            "job": {
                "settings": {
                    "BASE_TEST_REPOS": (
                        f"{settings.obs_download_url}/SUSE:/Maintenance:/1111/SUSE_Updates_SLE-Module-Basesystem_15-SP5_x86_64/,"
                        f"{settings.obs_download_url}/SUSE:/Maintenance:/{request.param}/SUSE_Updates_SLE-Module-Basesystem_15-SP5_x86_64/"
                    )
                },
            },
        },
...
def test_some_jobs_marked_as_acceptable_for_via_openqa_comment(
...
@pytest.mark.parametrize(
    "fake_responses_for_unblocking_submissions_via_openqa_comments",
    [{"submission": 22, "not_acceptable_job_ids": [100002]}],
    indirect=True,
)
@pytest.mark.usefixtures(
    "fake_responses_for_unblocking_submissions_via_openqa_comments",
...
def test_approval_still_blocked_if_openqa_comment_not_relevant(
    ...
    assert approver(mocker) == 0
    assert "SUSE:Maintenance:2:200 has at least one not-ok job in submission tests" in caplog.messages
