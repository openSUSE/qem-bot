...
def test_get_single_submission(mock_get_json: MagicMock) -> None:
    mock_get_json.return_value = {"number": 1, "rr_number": 123, "type": "smelt"}
    ...
    mock_get_json.assert_called_once_with("api/incidents/1", headers={}, params={"type": "smelt"})
...
def test_get_submission_settings_data(mock_get_json: MagicMock) -> None:
    ...
    mock_get_json.assert_called_once_with("api/incident_settings/1", headers={}, params={"type": "smelt"})
...
def test_get_active_submissions_with_type(mock_get_json: MagicMock) -> None:
    ...
    assert res == [123]
    mock_get_json.assert_called_once_with("api/incidents", headers={"token": "foo"}, params={"type": "smelt"})
...
def test_get_single_submission_with_type(mock_get_json: MagicMock) -> None:
    mock_get_json.return_value = {"number": 123, "rr_number": 456, "type": "smelt"}
    res = get_single_submission({"token": "foo"}, 123, submission_type="smelt")
    assert len(res) == 1
    assert res[0].sub == 123
    mock_get_json.assert_called_once_with("api/incidents/123", headers={"token": "foo"}, params={"type": "smelt"})
