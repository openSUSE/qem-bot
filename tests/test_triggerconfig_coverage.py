...
def test_trigger_config_obs_repo_url() -> None:
    ...
    config = TriggerConfig(
        ...
        repo_template="{repo_prefix}/{pr_id}",
    )
    assert config.generate_obs_repo_url(123, "http://obs", is_maintenance=False) == "http://obs/123"
    ...
    assert config_opensuse.generate_obs_repo_url(123, "http://obs", is_maintenance=True) == "http://obs/15.5/123"
